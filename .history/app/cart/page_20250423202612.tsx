...
const CheckoutForm = ({
  ...
  const paymentMethodOrder = {
    ...
    pay_later: ["afterpay_clearpay", "klarna", "card"],
  };
  ...
  return (
    <form onSubmit={handleSubmit}>
      <PaymentElement
        ...
        options={{
          layout: "tabs",
          paymentMethodOrder: paymentMethodOrder[paymentType],
          wallets:
            paymentType === "wallet"
      ...
      try {
        ...
        const res = await fetch("/api/stripe/checkout-session", {
          ...
          body: JSON.stringify({
            total,
            promoCode,
            paymentType: "card", // Hardcode to card
            billingDetails,
          }),
        });
        const data = await res.json();
  ...
  const applyPromoCode = async () => {
    ...
    try {
      const res = await fetch("/api/stripe/checkout-session", {
        method: "POST",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({ total, promoCode, paymentType: "card" }),
      });
      const data = await res.json();
      if (data.clientSecret) {
