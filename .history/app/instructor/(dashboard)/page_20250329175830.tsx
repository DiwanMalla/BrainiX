      ...
      if (user && isLoaded) {
        try {
          ...
          if (!user.roles.includes("instructor")) {
            // Redirect or show an error if the user is not an instructor
            alert("You must be an instructor to view this page.");
            return;
          }
          ...
          const instructorData = await prisma.user.findUnique({
            where: {
              userId: user.id, // Assuming the user ID is stored in the instructor table
            },
            include: {
