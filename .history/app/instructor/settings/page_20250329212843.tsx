...
export default function InstructorSettingsPage() {
  ...
  // State for preferences (loaded from local storage)
  ...
  const handleSaveProfile = async (data: SettingsForm) => {
    ...
    try {
      ...
      if (data.avatarUrl && data.avatarUrl !== user.imageUrl) {
        await user.update({ profileImageUrl: data.avatarUrl });
      }
  ...
  const handleSavePreferences = () => {
    ...
    localStorage.setItem("instructorPreferences", JSON.stringify(preferences));
    toast({ title: "Preferences saved!" });
  };

  const handleSaveDefaults = (data: SettingsForm) => {
    const defaults = {
      defaultPrice: Number(data.defaultPrice),
      defaultVisibility,
      defaultDiscount: Number(data.defaultDiscount),
    };
    localStorage.setItem("instructorDefaults", JSON.stringify(defaults));
