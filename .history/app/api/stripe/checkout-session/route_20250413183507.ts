...
export async function POST(request: Request) {
  ...
  try {
    const { total, promoCode, paymentType, billingDetails } =
      await request.json();
    ...
    // Create PaymentIntent without creating Order yet
    const paymentIntent = await stripe.paymentIntents.create({
      ...
      metadata: {
        ...
        promoCode: promoCode || "",
        billingDetails: JSON.stringify(billingDetails), // Store temporarily
      },
    });
