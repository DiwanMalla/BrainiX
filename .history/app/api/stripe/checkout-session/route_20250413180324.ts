...
export async function POST(request: Request) {
  ...
  try {
    const { total, promoCode, paymentType } = await request.json();
    ...
    // Verify client-provided total (allow small float discrepancies)
    ...
    const amount = Math.round(calculatedTotal * 100); // Convert to cents
    if (amount <= 0) {
      return NextResponse.json(
    ...
    // Define payment method types based on paymentType
    const paymentMethodTypes: string[] = (() => {
      switch (paymentType) {
        case "wallet":
          return ["card", "apple_pay", "google_pay"];
        case "pay_later":
          return ["afterpay_clearpay", "klarna"];
        case "card":
        default:
          return ["card"];
      }
