...
export async function POST(req: NextRequest) {
  try {
    ...
    if (
      (error instanceof Error && (error as any)?.response?.status === 403) ||
      ...
      (error instanceof Error && (error as any)?.response?.status === 429) ||
      ...
      (error instanceof Error && (error as any)?.response?.status === 400) ||
