...
export async function POST(req: NextRequest) {
  try {
    ...
    const response = await axios.post(
      ...
      {
        model: "deepseek-chat", // Adjust model name as per DeepSeek's documentation
      ...
      },
      {
        headers: {
          ...
          Authorization: `Bearer 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`,
        },
      }
    );
    ...
    if (error.response?.status === 401) {
      ...
      return NextResponse.json(
        { error: "DeepSeek API rate limit exceeded. Try again later." },
        { status: 429 }
      );
    } else if (error.response?.status === 400) {
      return NextResponse.json(
        { error: "Invalid request to DeepSeek API. Please check your input." },
        { status: 400 }
      );
    }
    return NextResponse.json(
