...
export async function POST(req: NextRequest) {
  try {
    ...
    const response = await axios.post(
      ...
      {
        model: "meta-ai/llama-3.1-8b-instruct", // Choose an available model (check OpenRouter docs)
      ...
      },
      {
        headers: {
          ...
          "HTTP-Referer":
            process.env.NEXT_PUBLIC_APP_URL || "http://localhost:3000", // Your app URL
          "X-Title": "BrainiX E-Learning Platform", // Optional: App name
        },
      }
    );
    ...
    if (error.response?.status === 401) {
      ...
      return NextResponse.json(
        {
          error: "Invalid request to OpenRouter API. Please check your input.",
        },
        { status: 400 }
      );
    }
