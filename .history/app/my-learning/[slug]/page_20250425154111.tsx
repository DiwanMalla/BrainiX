...
export default function CourseLearningPage() {
  ...
  const [showChat, setShowChat] = useState(false); // Disable chat temporarily
  ...
  useEffect(() => {
    ...
    const fetchNotes = async () => {
      try {
        ...
        const data = await res.json();
        setNotes(data);
        console.log("useEffect: Notes fetched", { notes: data });
      } catch (err: any) {
  ...
  const markLessonComplete = async () => {
    if (!course || !currentLesson) {
      console.log("markLessonComplete: No course or lesson available", {
        course,
        currentLesson,
      });
      toast({
    ...
    try {
      ...
      setCourse((prev) => {
        ...
        console.log("markLessonComplete: Course state updated", { newModules });
