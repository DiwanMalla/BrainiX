...
export default function CheckoutPage() {
  ...
  const [discount, setDiscount] = useState(0); // Track discount from server
  ...
  useEffect(() => {
    ...
    const fetchCart = async () => {
      ...
      try {
        ...
        toast({
          title: "Error",
          description: `Failed to load cart items ${err}`,
          variant: "destructive",
        });
      } finally {
        setIsLoading(false);
    ...
    const fetchClientSecret = async () => {
      ...
      try {
        ...
        toast({
          ...
          description: `Failed to connect to payment service. ${err}`,
          variant: "destructive",
        });
      }
    };
  ...
  const applyPromoCode = async () => {
    ...
    try {
      const res = await fetch("/api/stripe/checkout-session", {
        method: "POST",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({ total: subtotal, promoCode }),
      });
      const data = await res.json();
      if (data.clientSecret && data.discount !== undefined) {
      ...
      toast({
        title: "Error",
        description: `Failed to validate promo code: ${err}`,
        variant: "destructive",
      });
    }
  };
