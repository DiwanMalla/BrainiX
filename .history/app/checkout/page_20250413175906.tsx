...
// Define CartItem type based on Prisma schema
interface CartItem {
  id: string;
  courseId: string;
  userId: string;
...
const CheckoutForm = ({
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (stripeError) {
        ...
        // Clear cart and add to purchased courses via API
        await fetch("/api/cart", { method: "DELETE" });
        await fetch("/api/purchased-courses", {
          method: "POST",
          headers: { "Content-Type": "application/json" },
          body: JSON.stringify({ clientSecret }), // Pass clientSecret to verify
        });
  ...
  const paymentMethodTypes = {
    card: ["card"],
    wallet: ["card", "apple_pay", "google_pay", "paypal"],
...
export default function CheckoutPage() {
  ...
  useEffect(() => {
    ...
    const fetchClientSecret = async () => {
      try {
        const res = await fetch("/api/stripe/checkout-session", {
          method: "POST",
          headers: { "Content-Type": "application/json" },
          body: JSON.stringify({ total, promoApplied, paymentType }),
        });
        const data = await res.json();
    ...
    fetchClientSecret();
  }, [cartItems, total, promoApplied, user, paymentType, toast]);
  ...
  const handleCollapsibleToggle = (type: "card" | "wallet" | "pay_later") => {
    ...
    setClientSecret(null); // Reset clientSecret to fetch new PaymentIntent
  };
                  ...
                  <div className="space-y-3">
                    ...
                    {promoApplied && (
                      <div className="flex justify-between text-green-600 dark:text-green-400">
                        ...
                        <span>
                          -A${((subtotal * discount) / 100).toFixed(2)}
                        </span>
                      </div>
                    )}
                    <Separator />
