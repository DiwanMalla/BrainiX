...
const CheckoutForm = ({
  ...
  const [promoCode, setPromoCode] = useState(""); // Ensure promoCode is available
      ...
      const { error: stripeError, paymentIntent } = await stripe.confirmPayment(
        {
          ...
          redirect: "if_required", // Prevent redirect to handle order creation
        }
      );
  ...
  const applyPromoCode = async () => {
    ...
    try {
      const res = await fetch("/api/stripe/checkout-session", {
        method: "POST",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({ total, promoCode }),
      });
      const data = await res.json();
      if (data.clientSecret) {
  ...
  const handleCollapsibleToggle = (type: "card" | "wallet" | "pay_later") => {
    if (type === "pay_later") {
      toast({
        ...
        description: "Pay Later options are not yet available.",
        variant: "default",
      });
      return;
    }
    if (type === "wallet") {
      toast({
        title: "Feature In Progress",
        description: "Digital Wallet options are not yet available.",
        variant: "default",
      });
      return;
    }
        ...
        <div className="container px-4 md:px-6 max-w-6xl">
          ...
          <div className="grid gap-8 lg:grid-cols-3">
            <div className="lg:col-span-2">
                ...
                <CardContent className="space-y-6">
                  ...
                  {clientSecret ? (
                    <Elements stripe={stripePromise} options={{ clientSecret }}>
                      ...
                      >
                        <CollapsibleTrigger className="flex items-center gap-2 w-full text-left font-medium py-2 px-4 rounded-md opacity-50 cursor-not-allowed">
                      ...
                      {/* <Collapsible
                        open={false}
                        onOpenChange={() => handleCollapsibleToggle("pay_later")}
                      >
                        <CollapsibleTrigger className="flex items-center gap-2 w-full text-left font-medium py-2 px-4 rounded-md opacity-50 cursor-not-allowed">
                      ...
                      <div className="mt-4">
                        <CheckoutForm
                          ...
                          billingDetails={billingDetails}
                          promoCode={promoCode} // Pass promoCode
                        />
                      </div>
                    </Elements>
                  ) : error ? (
              ...
              {/* Save Info and Newsletter Options */}
              <Card className="mb-6">
                      ...
                      <div key={item.id} className="flex gap-3">
                        ...
                        <div className="flex-1 min-w-0">
                          ...
                          <p className="text-sm font-bold mt-1">
                            A${(item.discountPrice || item.price).toFixed(2)}
                          </p>
                        </div>
                      </div>
                    ))}
