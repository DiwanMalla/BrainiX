...
export default function CategoryDetailPage() {
  ...
  const filteredCourses =
    category?.courses.filter((course) => {
      ...
      // Note: This assumes courses are tied to subcategories indirectly; adjust if your schema links them directly
      const matchesSubcategory =
        selectedSubcategory === "all" ||
        category.subcategories.some(
          (s) => s.id === selectedSubcategory && course.slug.includes(s.slug)
        );
  ...
  const sortedCourses = [...filteredCourses].sort((a, b) => {
    switch (sortOption) {
      case "newest":
        return 0; // Adjust if you add a createdAt field
      case "rating":
