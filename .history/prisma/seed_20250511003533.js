...
async function main() {
  ...
  // Ensure the course exists, create if not, including required price field
  const course = await prisma.course.upsert({
    where: { slug: "business-analytics-fundamentals" },
    update: {}, // Empty update is fine since we’re not modifying existing courses
    create: {
      ...
      price: 99.99, // Added required price field (adjust as needed)
    },
  });

  console.log(
    `Course ensured: ${course.title} (Slug: ${course.slug}, Price: ${course.price})`
  );
