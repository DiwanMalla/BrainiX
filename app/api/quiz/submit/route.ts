...
export async function POST(req: Request) {
  try {
    ...
    const answerRecords: { questionId: string; selectedAnswer: string }[] = [];

    for (const question of quiz.questions) {
      const submittedAnswer = answers[question.id];
      if (!submittedAnswer) {
        console.warn("Quiz Submit: Missing answer for question", {
          questionId: question.id,
        });
        answerRecords.push({ questionId: question.id, selectedAnswer: "" });
        continue;
      }
